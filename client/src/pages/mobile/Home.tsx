...
export default function MobileHome() {
  ...
  const recentNews = MOCK_NEWS.slice(0, 2);

  return (
    <MobileLayout>
      <div className="p-6 pt-12 bg-white relative">
        {/* Welcome Header */}
        <div className="flex justify-between items-start mb-8">
          <div>
            ...
            <div className="flex items-center gap-2 mt-2">
              <Badge variant="secondary" className="bg-green-100 text-green-700 hover:bg-green-100 px-3 py-1 rounded-full text-xs font-semibold border-0">
                Adhérent Actif
              </Badge>
              <span className="text-xs text-gray-400">{user.section}</span>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-primary to-blue-700 rounded-2xl p-6 text-white shadow-lg shadow-blue-900/20 mb-8 transform transition-transform active:scale-98 cursor-pointer relative overflow-hidden group">
            ...
            <div className="flex justify-between items-start relative z-10">
              <div>
                ...
                <p className="text-2xl font-mono tracking-wider font-bold text-white/90">{user.memberId}</p>
              </div>
